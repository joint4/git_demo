...
<link href="<?php echo CUSTOM_TEMPLATE_PATH;?>Index/bigBackgroundV1/main.css?v=<?php echo SITE_VERSION;?>" rel="stylesheet" type="text/css">
...
<div class="container">
    <!--<?php if(!empty($slideshow)): ?><section class="banner">
    	<ul>
        <?php if(is_array($slideshow)): $i = 0; $__LIST__ = $slideshow;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><a href="<?php echo ($vo["url"]); ?>"><img src="<?php echo ($vo["img"]); ?>"/></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
        <span class="identify">
    ...
    <?php if(!empty($category)): ?><section>
    	<ul class="icon_lists">
            <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="m_15">
                <a class="item p_10" href="<?php echo ($vo["url"]); ?>">
                    <?php if(!empty($vo[icon])): ?><span class="icon"><img src="<?php echo ($vo["icon"]); ?>"/></span><?php endif; ?>
