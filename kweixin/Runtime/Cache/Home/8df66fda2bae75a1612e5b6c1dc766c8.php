      ...
      <div class="table-bar">
        <div class="fl">
          <?php if(empty($model["extend"])): ?><div class="tools">
				<?php if($add_button): $add_url || $add_url = U('listsadd?model='.$model['id']); ?><a class="btn" href="<?php echo ($add_url); ?>">新增餐厅</a><?php endif; ?>
				<?php if($del_button): $del_url || $del_url = U('listsdel?model='.$model['id']); ?><button class="btn ajax-post confirm" target-form="ids" url="<?php echo ($del_url); ?>">删 除</button><?php endif; ?>                
			</div><?php endif; ?>
        </div>
        <!-- 高级搜索 -->
